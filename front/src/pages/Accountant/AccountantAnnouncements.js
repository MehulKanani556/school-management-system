        ...
        >
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                <div className="space-y-3">
                    <h1 className="text-4xl font-black text-white italic uppercase tracking-tighter leading-none font-outfit">Fiscal Bulletin</h1>
                    <p className="text-slate-500 font-medium text-lg italic">Sector-wide broadcast archive and public notices.</p>
                </div>
                
                <div className="flex bg-slate-900 p-1 rounded-md border border-slate-800/60 shadow-2xl">
                    <button 
                        onClick={() => setActiveTab('announcements')}
                        className={`flex items-center gap-3 px-8 py-3.5 rounded-md text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'announcements' ? 'bg-accountant-primary text-black shadow-xl' : 'text-slate-500 hover:text-slate-300'}`}
                    >
                        <Megaphone size={16} /> Broadcasts
                    </button>
                    <button 
                        onClick={() => setActiveTab('notices')}
                        className={`flex items-center gap-3 px-8 py-3.5 rounded-md text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'notices' ? 'bg-slate-800 text-white shadow-xl' : 'text-slate-500 hover:text-slate-300'}`}
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start flex-1 min-h-0">
                ...
                <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-24">
                    <div className="bg-slate-900 border border-slate-800/60 p-8 rounded-md shadow-2xl space-y-8">
                        <div>
                            <h3 className="text-sm font-black text-white uppercase tracking-widest mb-6 flex items-center gap-3">
                                <span className="w-8 h-px bg-accountant-primary"></span> Signal Scan
                            </h3>
                            <div className="relative group">
                                <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-700 group-focus-within:text-accountant-primary transition-colors" size={18} />
                                <input 
                                    type="text"
                                    placeholder="SCAN KEYWORDS..."
                                    value={searchTerm}
                                    onChange={(e) => setSearchTerm(e.target.value)}
                                    className="w-full bg-slate-950/50 border border-slate-800 h-14 pl-12 pr-6 rounded-md text-[10px] font-black uppercase tracking-[0.2em] text-white outline-none focus:border-accountant-primary placeholder:text-slate-800 italic transition-all shadow-inner"
                                />
                            </div>
                        </div>

                        <div className="pt-8 border-t border-slate-800/50 space-y-4">
                            <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic">Node Reliability</h4>
                            <div className="flex items-center gap-3 text-accountant-primary text-[11px] font-bold italic">
                                <CheckCircle size={14} />
                                <span>Encrypted Signal Active</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <AnimatePresence mode="popLayout">
                        {loading ? (
                            <div className="py-40 flex flex-col items-center justify-center space-y-6">
                                <div className="w-12 h-12 border-4 border-accountant-primary/20 border-t-accountant-primary rounded-md animate-spin shadow-[0_0_20px_rgba(245,158,11,0.2)]"></div>
                            ...
                            displayItems.map((item, idx) => (
                                <motion.div 
                                    ...
                                    className={`bg-slate-900 border-l-4 p-10 rounded-md shadow-2xl relative overflow-hidden group hover:bg-slate-800/20 transition-all ${activeTab === 'announcements' ? 'border-l-accountant-primary border-slate-800/40' : 'border-l-slate-600 border-slate-800/40'}`}
                                >
                                    ...
                                    <div className="relative z-10 flex flex-col md:flex-row justify-between items-start gap-6 mb-8">
                                        <div className="flex-1">
                                            <div className="flex items-center gap-4 mb-4">
                                                <span className={`px-4 py-1.5 rounded-md text-[9px] font-black uppercase tracking-widest border ${activeTab === 'announcements' ? 'bg-accountant-primary/10 text-accountant-primary border-accountant-primary/20' : 'bg-slate-800/10 text-slate-400 border-slate-800/20'}`}>
                                                    {activeTab === 'announcements' ? 'Institutional Signal' : 'Public Notice'}
                                                </span>
                                    ...
                                    <div className="relative z-10 flex items-center justify-between pt-6 border-t border-slate-800/50">
                                        ...
                                        <button className="flex items-center gap-3 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-white transition-all italic group/btn">
                                            Archive Directive <ArrowUpRight size={14} className="group-hover/btn:translate-x-1 group-hover/btn:-translate-y-1 transition-transform" />
                                        </button>
                                    </div>
                                </motion.div>
                            ))
                        ) : (
                            <div className="py-40 text-center bg-slate-900/40 rounded-md border border-slate-800/50 border-dashed">
