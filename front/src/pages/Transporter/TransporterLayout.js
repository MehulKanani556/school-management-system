...
const navItems = [
    { to: '/transporter', icon: LayoutDashboard, label: 'Fleet Control', end: true },
    {
        label: 'Logistics',
        icon: Truck,
        children: [
            { to: '/transporter/vehicles', icon: Truck, label: 'Vehicle Fleet' },
            { to: '/transporter/routes', icon: Route, label: 'Vector Routes' },
            { to: '/transporter/tracking', icon: Navigation, label: 'Live Tracking' },
        ]
    },
    { to: '/transporter/students', icon: Users, label: 'Passenger Log' },
    { to: '/transporter/messages', icon: MessageSquare, label: 'Direct Probe' },
];
...
const TransporterLayout = () => {
    ...
    useEffect(() => {
        ...
        socket.on('new_notification', (notif) => {
            ...
            toast.success(`Logistics Alert: ${notif.title}`, {
                icon: '🚚',
                style: {
                    borderRadius: '1.5rem',
                    background: '#0f172a',
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-slate-900 border-r border-slate-800/60 flex flex-col transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static lg:h-full`}>
                <div className="p-8 flex-shrink-0">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-md bg-gradient-to-br from-brand-primary to-brand-secondary flex items-center justify-center font-black text-xl italic shadow-lg">TR</div>
                        <span className="text-xl font-black tracking-tight uppercase font-outfit leading-none">Vehicle <span className="text-brand-primary">Node</span></span>
                    </div>
                </div>
            ...
            <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
                ...
                <header className="h-20 flex-shrink-0 flex items-center justify-between px-8 bg-slate-900/60 backdrop-blur-xl border-b border-slate-800/40 z-10 w-full transition-all">
                    <div className="flex items-center gap-4 text-slate-500">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest bg-slate-800 px-3 py-1 rounded-md border border-slate-700/50 hidden sm:block leading-none">Logistics Intelligence</span>
                        <ChevronRight size={14} className="hidden sm:block" />
                        <span className="text-[10px] font-black uppercase tracking-widest text-brand-primary">Transporter Terminal</span>
                    </div>
                        ...
                        <div className="flex items-center gap-4 relative">
                            ...
                            >
                                <div className="text-right hidden sm:block">
                                    ...
                                    <p className="text-[9px] font-black text-brand-primary uppercase tracking-[0.2em] mt-1.5 opacity-80 leading-none italic">Logistics Commander</p>
                                </div>
