      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input 
            ...
            className="w-full bg-brand-surface/40 border border-brand-border/40 rounded-md py-3.5 pl-11 pr-5 text-white placeholder-slate-600 outline-none focus:border-brand-primary transition-all shadow-inner shadow-black/20" 
          />
        </div>
        <div className="flex p-1.5 bg-brand-surface/60 rounded-md border border-brand-border/40">
          {['all', 'pending', 'approved', 'rejected'].map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-5 py-2 rounded-md text-[10px] font-black uppercase tracking-widest transition-all ${filter === f ? 'bg-brand-primary text-white shadow-lg shadow-blue-600/20' : 'text-slate-500 hover:text-white'}`}
            >
              {f}
            </button>
          ))}
        ...
        {loading && leaves.length === 0 ? (
          ...
          filtered.map((l, i) => (
            <motion.div
              ...
              className="bg-brand-surface/40 backdrop-blur-xl border border-brand-border/30 rounded-md p-6 lg:p-8 hover:border-brand-primary/40 transition-all group shadow-xl shadow-black/20"
            >
