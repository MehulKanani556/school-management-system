        ...
        <div className="min-h-screen bg-[#020617] text-slate-300 font-outfit p-4 lg:p-8 selection:bg-brand-primary/30 selection:text-white overflow-hidden flex flex-col">
            ...
            <div className="max-w-[1600px] w-full mx-auto mb-8 shrink-0">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    ...
                    <div className="flex bg-slate-900/40 p-1 rounded-md border border-slate-800/40 backdrop-blur-3xl shadow-2xl">
                        {[
                            ...
                            <button 
                                ...
                                className={`flex items-center gap-3 px-6 py-3.5 rounded-md text-[10px] font-black uppercase tracking-widest transition-all relative overflow-hidden group ${activeTab === tab.id ? `bg-${tab.color} text-white shadow-xl shadow-${tab.color}/20 scale-105` : 'text-slate-500 hover:text-slate-300'}`}
                            >
                                <tab.icon size={16} />
                                {tab.label}
                            </button>
                ...
                {activeTab === 'messages' ? (
                                ...
                                <div className="p-8 border-b border-slate-800/60">
                                    <div className="relative group">
                                        ...
                                        <input 
                                            ...
                                            className="w-full bg-slate-950/50 border border-slate-800 h-14 pl-12 pr-6 rounded-md text-[10px] font-black uppercase tracking-widest text-white outline-none focus:border-brand-primary placeholder:text-slate-700 italic"
                                        />
                                    </div>
                                </div>
                                    ...
                                    {conversations.map(conv => {
                                        ...
                                        return (
                                            <button 
                                                ...
                                                className={`w-full flex items-center gap-4 p-4 rounded-md transition-all border group ${isActive ? 'bg-brand-primary/10 border-brand-primary/30' : 'bg-transparent border-transparent hover:bg-slate-800/30'}`}
                                            >
                                                <div className="w-14 h-14 rounded-md bg-slate-800 border border-white/5 overflow-hidden shadow-lg relative shrink-0">
                                                    {p.photo ? <img src={p.photo} alt="" className="w-full h-full object-cover" /> : <User className="w-full h-full p-3 text-slate-600" />}
                                                    <div className="absolute top-1 right-1 w-3 h-3 bg-emerald-500 rounded-md border-2 border-slate-900 shadow-xl"></div>
                                                </div>
                                                ...
                                                <div className="flex flex-col items-end gap-2">
                                                    ...
                                                    {conv.messages.some(m => !m.isRead) && <div className="w-2 h-2 rounded-md bg-luxury-rose animate-pulse shadow-rose-500/50 shadow-lg"></div>}
                                                </div>
                                            </button>
                                        );
                                    })}
                                ...
                                <>
                                    ...
                                    <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
                                        <div className="flex items-center gap-5">
                                            <button onClick={() => setSelectedChat(null)} className="lg:hidden p-3 rounded-md bg-slate-800/60 text-slate-400 mr-2"><ArrowLeft size={20}/></button>
                                            <div className="w-14 h-14 rounded-md bg-slate-800 overflow-hidden border border-brand-primary/20 shadow-xl shadow-brand-primary/5">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <button className="p-4 rounded-md border border-slate-800 text-slate-500 hover:text-white hover:bg-slate-800/50 transition-all"><Search size={18} /></button>
                                            <button className="p-4 rounded-md border border-slate-800 text-slate-500 hover:text-white hover:bg-slate-800/50 transition-all"><AlertCircle size={18} /></button>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex-1 overflow-y-auto p-10 space-y-8 custom-scrollbar bg-slate-950/20">
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div className={`max-w-[75%] space-y-2 ${isMe ? 'items-end' : 'items-start'} flex flex-col`}>
                                                        <div className={`p-6 rounded-md text-sm font-bold shadow-2xl relative ${isMe ? 'bg-brand-primary text-white rounded-tr-md' : 'bg-slate-800 text-slate-200 rounded-tl-md border border-white/5'}`}>
                                                            ...
                                                            {i === activeConversation.messages.length - 1 && isMe && (
                                                                <div className="absolute -bottom-6 right-2 flex items-center gap-2">
                                                                    <div className="h-2 w-2 rounded-md bg-brand-primary/40 animate-pulse"></div>
                                        ...
                                        {(!activeConversation || activeConversation.messages.length === 0) && (
                                            <div className="h-full flex flex-col items-center justify-center opacity-30 gap-6">
                                                <div className="w-20 h-20 rounded-md border-2 border-dashed border-slate-700 flex items-center justify-center">
                                    ...
                                    <div className="p-8 border-t border-white/5 bg-slate-900 shadow-[0_-20px_50px_rgba(0,0,0,0.5)]">
                                        <form onSubmit={(e) => { e.preventDefault(); handleSend(null, selectedChat); }} className="flex items-center gap-4 bg-slate-950/50 border border-slate-800 rounded-md p-2.5 focus-within:border-brand-primary transition-all">
                                            <button type="button" className="p-4 rounded-md text-slate-600 hover:text-white transition-colors"><Paperclip size={20} /></button>
                                            <input 
                                                required
                                            ...
                                            />
                                            <button type="button" className="p-4 rounded-md text-slate-600 hover:text-white transition-colors"><Smile size={20} /></button>
                                            <button 
                                                type="submit"
                                                className="bg-brand-primary text-white p-4 rounded-md shadow-lg shadow-brand-primary/20 hover:scale-105 transition-all group active:scale-95"
                                            >
                    ...
                    <>
                        ...
                        <div className="lg:col-span-5 flex flex-col min-h-0 min-w-0">
                            <div className="bg-slate-900/40 border border-slate-800/60 rounded-md p-10 backdrop-blur-3xl relative overflow-hidden group shadow-2xl flex-1 flex flex-col min-h-0">
                                <div className="absolute top-0 right-0 w-80 h-80 bg-brand-primary/5 rounded-md blur-[120px] -mr-40 -mt-40 transition-transform group-hover:scale-110 duration-1000"></div>
                                ...
                                <div className="relative z-10 space-y-8 flex flex-col h-full">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest bg-slate-800/50 px-4 py-1.5 rounded-md border border-slate-700/50 italic">SECURE DISPATCH</div>
                                    </div>
                                    ...
                                    <form onSubmit={handleSend} className="space-y-8 flex-1 flex flex-col min-h-0 pr-2 custom-scrollbar overflow-y-auto">
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    {['All', 'Student', 'Teacher', 'Parent'].map(role => (
                                                        <button
                                                            ...
                                                            className={`py-4 rounded-md text-[11px] font-black uppercase tracking-widest border transition-all ${formData.targetRole === role ? 'bg-brand-primary/10 border-brand-primary text-brand-primary shadow-lg shadow-brand-primary/5 scale-[1.02]' : 'bg-slate-950/60 border-slate-800 text-slate-600 hover:border-slate-700'}`}
                                                        >
                                                            {role}
                                                        </button>
                                                    ))}
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <input 
                                                ...
                                                className={`w-full bg-slate-950/60 border border-slate-800 rounded-md p-6 text-white text-sm font-black uppercase tracking-tighter outline-none focus:border-${activeTab === 'announcements' ? 'brand-primary' : 'emerald-500'} transition-all placeholder:text-slate-800 italic`}
                                            />
                                        </div>
                                        ...
                                        <div className="space-y-4 flex-1 flex flex-col">
                                            ...
                                            <textarea 
                                                ...
                                                value={formData.content}
                                                onChange={(e) => setFormData({...formData, content: e.target.value})}
                                                className={`flex-1 w-full bg-slate-950/60 border border-slate-800 rounded-md p-8 text-white text-sm font-bold outline-none focus:border-${activeTab === 'announcements' ? 'brand-primary' : 'emerald-500'} transition-all placeholder:text-slate-800 italic resize-none uppercase tracking-tighter`}
                                            />
                                        </div>

                                        <button 
                                            type="submit"
                                            className={`w-full py-6 rounded-md flex items-center justify-center gap-4 text-[14px] font-black uppercase tracking-[0.3em] transition-all shadow-2xl active:scale-95 group ${activeTab === 'announcements' ? 'bg-brand-primary text-white shadow-brand-primary/20 hover:bg-brand-primary/90' : 'bg-emerald-500 text-white shadow-emerald-500/20 hover:bg-emerald-600'}`}
                                        >
                            ...
                            <div className="flex items-center justify-between shrink-0 px-2">
                                <div className="flex items-center gap-5">
                                    <h3 className="text-sm font-black text-white uppercase tracking-widest flex items-center gap-4 italic leading-none">
                                        <div className={`p-2 rounded-md bg-${activeTab === 'announcements' ? 'brand-primary' : 'emerald-500'}/10 border border-${activeTab === 'announcements' ? 'brand-primary' : 'emerald-500'}/20`}>
                                            <Filter size={20} className={`text-${activeTab === 'announcements' ? 'brand-primary' : 'emerald-500'}`} />
                                        </div>
                                        ACTIVE TRANSMISSION ARCHIVE
                                    </h3>
                                    <span className="bg-slate-900 border border-slate-800 text-slate-500 text-[9px] font-black px-4 py-1.5 rounded-md tracking-widest italic group-hover:text-white transition-colors cursor-default">
                                ...
                                <AnimatePresence mode="popLayout">
                                    {(activeTab === 'announcements' ? announcements : notices).map((item, idx) => (
                                        <motion.div 
                                            ...
                                            key={item._id} 
                                            className={`bg-slate-900/30 border border-slate-800/60 rounded-md p-10 hover:border-white/20 transition-all group relative overflow-hidden backdrop-blur-2xl shadow-xl ${activeTab === 'notices' ? 'border-l-4 border-l-emerald-500/40' : 'border-l-4 border-l-brand-primary/40'}`}
                                        >
                                            <div className="absolute top-0 right-0 w-40 h-40 bg-white/[0.02] rounded-md -mr-20 -mt-20"></div>
                                            
                                            <div className="flex flex-col md:flex-row justify-between items-start gap-6 mb-8 relative z-10">
                                                <div className="flex items-center gap-6">
                                                    <div className={`w-16 h-16 rounded-md flex items-center justify-center border border-white/5 shadow-2xl ${activeTab === 'announcements' ? 'bg-brand-primary/10 text-brand-primary shadow-brand-primary/5' : 'bg-emerald-500/10 text-emerald-500 shadow-emerald-500/5'}`}>
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="flex items-center gap-4">
                                                            <span className={`text-[10px] font-black uppercase tracking-widest px-4 py-1.5 rounded-md border italic shadow-inner ${activeTab === 'announcements' ? 'text-brand-primary bg-brand-primary/5 border-brand-primary/10' : 'text-emerald-400 bg-emerald-500/5 border-emerald-500/10'}`}>
                                                                {activeTab === 'announcements' ? `TARGET: ${item.targetRole}` : 'GLOBAL BULLETIN'}
                                                            </span>
                                                            <div className="flex items-center gap-2 text-[10px] font-bold text-slate-600 uppercase tracking-widest italic bg-slate-950/50 px-3 py-1.5 rounded-md border border-white/5">
                                                                <Calendar size={12} className="text-slate-700" /> {new Date(item.createdAt).toLocaleDateString()}
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <button onClick={() => handleDelete(item._id)} className="p-4 rounded-md bg-slate-950 border border-slate-800 flex items-center justify-center text-slate-700 hover:text-rose-500 hover:border-rose-500/30 transition-all opacity-0 group-hover:opacity-100 shadow-2xl active:scale-90">
                                                    <Trash2 size={20} />
                                                </button>
                                            </div>
                                            ...
                                            <div className="flex items-center justify-between text-[11px] font-black uppercase tracking-[0.2em] text-slate-600 italic relative z-10 pt-6 border-t border-white/[0.03]">
                                                <div className="flex items-center gap-3">
                                                    <div className={`w-2 h-2 rounded-md ${activeTab === 'announcements' ? 'bg-brand-primary' : 'bg-emerald-500'} animate-pulse`}></div>
