  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={openAdd} className="flex items-center gap-2 px-5 py-3 bg-brand-primary hover:bg-blue-500 rounded-2xl font-black text-sm uppercase tracking-wider transition-all font-outfit text-white">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {loading && subjects.length === 0 ? (
          [...Array(6)].map((_, i) => <div key={i} className="h-32 rounded-[2rem] bg-slate-800/30 animate-pulse" />)
        ) : subjects.length === 0 ? (
            ...
            <div className="flex items-start justify-between relative z-10">
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {sub.code && (
                    <span className="px-2 py-0.5 bg-brand-primary/10 text-brand-primary text-[10px] font-black rounded-lg uppercase">
                      {sub.code}
                    </span>
                  )}
                </div>
              ...
              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => openEdit(sub)} className="p-2 rounded-xl hover:bg-brand-primary/20 text-slate-500 hover:text-brand-primary transition-all">
                  <Pencil size={14} />
                </button>
                <button onClick={() => dispatch(deleteSubject(sub._id))} className="p-2 rounded-xl hover:bg-red-500/10 text-slate-500 hover:text-red-400 transition-all">
                  <Trash2 size={14} />
                </button>
              </div>
            </div>
