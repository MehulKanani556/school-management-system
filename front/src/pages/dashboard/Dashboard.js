...
const Dashboard = () => {
    ...
    return (
        <div className="min-h-screen bg-brand-background text-white flex flex-col relative overflow-hidden font-inter">
            {/* Background Orbs */}
            <div className="absolute top-[-20%] left-[-10%] w-[60%] h-[60%] bg-brand-primary/10 rounded-md blur-[150px]"></div>
            <div className="absolute bottom-[-20%] right-[-10%] w-[60%] h-[60%] bg-brand-secondary/10 rounded-md blur-[150px]"></div>
            ...
            <header className="px-8 py-6 flex justify-between items-center bg-brand-surface/40 backdrop-blur-xl border-b border-brand-border/40 relative z-10">
                <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-md bg-gradient-to-br from-brand-primary to-brand-secondary flex items-center justify-center font-black tracking-tighter text-2xl italic shadow-[0_8px_30px_rgb(37,99,235,0.2)]">SM</div>
                    <span className="text-2xl font-black tracking-tight uppercase font-outfit">School Management</span>
                </div>
                
                <div className="flex items-center gap-6">
                    <div className="flex items-center gap-4 px-5 py-2.5 bg-slate-800/20 rounded-md border border-brand-border/40">
                        ...
                        {user?.photo ? (
                            <img src={user.photo} alt="Avatar" className="w-11 h-11 rounded-md object-cover ring-2 ring-brand-primary/20 shadow-xl" />
                        ) : (
                            <div className="w-11 h-11 rounded-md bg-slate-800 flex items-center justify-center border border-brand-border/40">
                    ...
                    <button 
                        ...
                        className="p-3.5 rounded-md bg-slate-800/30 hover:bg-luxury-rose/10 border border-brand-border/40 hover:border-luxury-rose/30 text-slate-500 hover:text-luxury-rose transition-all active:scale-[0.95] group"
                    >
            ...
            <main className="flex-1 p-8 grid grid-cols-1 lg:grid-cols-4 gap-10 relative z-10">
                ...
                <aside className="hidden lg:block space-y-3">
                    {[
                        ...
                        <button 
                            key={idx}
                            className={`w-full flex items-center gap-4 px-7 py-4 rounded-md transition-all duration-300 ${item.active ? 'bg-brand-primary text-white shadow-[0_10px_25px_-5px_rgba(37,99,235,0.4)]' : 'text-slate-500 hover:bg-slate-800/30 hover:text-white'}`}
                        >
                            <item.icon size={22} />
                ...
                <div className="lg:col-span-3 space-y-10">
                    <motion.div 
                        ...
                        className="bg-brand-surface/40 backdrop-blur-3xl border border-brand-border/40 rounded-md p-12 shadow-2xl overflow-hidden relative"
                    >
                        ...
                        <div className="mt-16 p-10 bg-gradient-to-br from-brand-primary/10 to-transparent border border-brand-primary/20 rounded-md inline-block">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        <div className="bg-brand-surface/40 backdrop-blur-2xl border border-brand-border/40 rounded-md p-10 h-[340px] flex items-center justify-center border-dashed">
