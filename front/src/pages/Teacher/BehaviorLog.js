...
import { fetchAssignedClasses, fetchClassStudents, logBehavior, fetchBehaviorLogs } from '../../redux/slice/teacher.slice';
                ...
                <div className="space-y-6">
                    ...
                    {behaviorLogs?.slice(0, 5).map((log, i) => (
                        <motion.div 
                            ...
                            className="bg-slate-900 border border-slate-800 rounded-md p-6 group hover:border-brand-primary/30 transition-all border-l-4"
                        ...
                        >
                            ...
                            <p className="text-[10px] font-medium text-slate-500 mb-4 line-clamp-2 italic">"{log.description}"</p>
                            {log.actionTaken && (
            ...
            <AnimatePresence>
                {isModalOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-slate-950/90 backdrop-blur-xl" onClick={() => setIsModalOpen(false)} />
                        <motion.div initial={{ scale: 0.9, y: 30 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 30 }} className="bg-slate-900 border border-slate-800 w-full max-w-lg rounded-md overflow-hidden relative shadow-2xl z-10">
                            <div className="p-8 border-b border-slate-800 flex justify-between items-center bg-slate-900/50 backdrop-blur-xl">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h2 className="text-lg font-black uppercase font-outfit tracking-tight">Conduct Vector Injection</h2>
