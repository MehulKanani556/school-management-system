        ...
        <div className="space-y-12">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <div className="flex gap-4">
                    <div className="bg-slate-800/40 border border-slate-700/50 p-6 rounded-md min-w-[160px]">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                {quickStats.map((stat, idx) => (
                    <motion.div
                        ...
                        className="bg-slate-900/40 border border-slate-800 p-8 rounded-md relative group hover:border-brand-primary/40 transition-all shadow-xl"
                    >
                        ...
                        <div className="absolute top-8 right-8 w-2 h-2 rounded-md bg-slate-800 group-hover:bg-brand-primary transition-colors"></div>
                    </motion.div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {[1, 2].map(id => (
                            <div key={id} className="bg-gradient-to-br from-slate-900 via-slate-900 to-brand-primary/5 p-8 rounded-md border border-slate-800/80 shadow-2xl relative overflow-hidden group">
                                <div className="absolute -top-10 -right-10 w-32 h-32 bg-brand-primary/10 rounded-md blur-3xl opacity-0 group-hover:opacity-100 transition-opacity"></div>
                                <div className="flex items-start justify-between mb-8">
                                    <div className="w-12 h-12 rounded-md bg-slate-800 border border-slate-700/50 flex items-center justify-center font-black text-xl text-brand-primary font-outfit italic shadow-lg">0{id}</div>
                                    <span className="text-[10px] bg-slate-800 px-3 py-1 rounded-md font-black text-slate-400 uppercase tracking-widest border border-slate-700/50 italic">Sector Class</span>
                                </div>
                                ...
                                <div className="flex items-center gap-4">
                                  <Link to="/teacher/attendance" className="flex-1 py-3 text-center bg-brand-primary hover:bg-blue-600 rounded-md text-[10px] font-black uppercase tracking-widest transition-all shadow-lg active:scale-95">Attendance</Link>
                                  <Link to="/teacher/marks" className="flex-1 py-3 text-center bg-slate-800/80 hover:bg-slate-700 rounded-md text-[10px] font-black uppercase tracking-widest transition-all border border-slate-700/50 active:scale-95 text-slate-300">Marks</Link>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-slate-900/60 border border-slate-800 p-8 rounded-md h-full shadow-2xl">
                        <div className="flex items-center gap-3 mb-8">
                          <div className="w-2 h-2 rounded-md bg-brand-primary animate-ping"></div>
                          <span className="text-[11px] font-black uppercase text-slate-400 tracking-widest italic">Broadcast Signals</span>
                        </div>
                        <div className="space-y-6">
                            {[1, 2, 3].map(msg => (
                                <div key={msg} className="p-5 bg-slate-800/30 rounded-md border border-slate-700/30 hover:border-brand-primary/30 transition-all cursor-pointer group">
                        ...
                        <button className="w-full mt-10 py-4 bg-slate-800 hover:bg-slate-700 rounded-md text-[10px] font-black uppercase tracking-[0.4em] transition-all border border-slate-700/50 flex items-center justify-center gap-3">All Channels <ArrowRight size={14} /></button>
                    </div>
                </div>
            </div>
        </div>
