    ...
    return (
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-10 container">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-2 border-b border-white/5">
                <div>
                    <h1 className="text-4xl font-black text-white italic uppercase tracking-tighter leading-none mb-3 font-outfit">Homework & Assignments</h1>
                ...
                >
                    ...
                    {viewMode === 'list' ? 'Add Assignment' : 'Back to List'}
                </button>
            </header>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-2.5">
                                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-1">Class/Section</p>
                                ...
                                <div className="space-y-2.5">
                                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-1">Subject</p>
                                    <div className="relative group">
                                        <ChevronDown size={14} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 pointer-events-none" />
                            ...
                            <div className="space-y-2.5">
                                ...
                                <input type="text" placeholder="Assignment topic..." value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} required className="w-full bg-slate-800/40 border border-slate-700/50 h-14 px-6 rounded-md text-[11px] font-bold text-white outline-none focus:border-brand-primary transition-all shadow-inner" />
                            </div>
                            <div className="space-y-2.5">
                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-1">Description & Instructions</p>
                                <textarea placeholder="Enter assignment details and instructions..." value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} rows={4} className="w-full bg-slate-800/40 border border-slate-700/50 p-6 rounded-md text-[11px] font-medium text-slate-300 outline-none focus:border-brand-primary transition-all shadow-inner resize-none" />
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-2.5">
                                    <p className="text-[10px] font-black uppercase tracking-[0.2em] ml-1 text-luxury-rose">Due Date</p>
                                ...
                                <div className="space-y-2.5">
                                    ...
                                    <label className="flex items-center gap-4 bg-slate-800/40 border border-slate-700/50 border-dashed hover:border-brand-primary hover:bg-brand-primary/5 h-14 px-6 rounded-md cursor-pointer transition-all group">
                                        ...
                                        <span className={`${formData.file ? 'text-brand-primary' : 'text-slate-500'} text-[11px] font-black uppercase tracking-widest truncate max-w-[150px]`}>
                                            {formData.file ? formData.file.name : (editMode && editMode.fileUrl ? 'Re-upload File' : 'Choose File')}
                                        </span>
                            ...
                            <button type="submit" disabled={loading} className="w-full py-5 bg-brand-primary hover:bg-teacher-primary rounded-md text-[11px] font-black uppercase tracking-[0.4em] text-white shadow-2xl active:scale-[0.98] transition-all flex items-center justify-center gap-4 disabled:opacity-50">
                                ...
                                {editMode ? 'UPDATE ASSIGNMENT' : 'POST ASSIGNMENT'}
                            </button>
                        </form>
                    </motion.div>
                ) : viewMode === 'list' ? (
                    <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {assignments.length > 0 ? assignments.map((assignment) => (
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Std {assignment.classSection?.standardId?.level || assignment.classSection?.standardId?.gradeLevel} - {assignment.classSection?.sectionLabel}</span>
                                        </div>
                                    </div>
                            ...
                            <div className="col-span-full py-20 bg-slate-900/40 border-2 border-dashed border-slate-800 rounded-md flex flex-col items-center justify-center opacity-40">
                                ...
                                <p className="text-xs font-black uppercase tracking-[0.3em] text-slate-500">No Assignments Found</p>
                            </div>
                        )}
                    </motion.div>
                    ...
                    <motion.div key="submissions" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-10">
                        <div className="flex items-center gap-6 p-8 bg-brand-primary/10 border border-brand-primary/20 rounded-md shadow-inner">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] font-black text-brand-primary uppercase tracking-[0.3em] italic">Submission Tracking • {submissions.length} Students Submitted</p>
                            </div>
                        </div>
                            ...
                            {submissions.map((sub, idx) => (
                                <motion.div key={sub._id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: idx * 0.05 }} className="bg-slate-900/60 border border-slate-800/80 rounded-md p-8 shadow-2xl relative overflow-hidden group hover:border-brand-primary/40 transition-all backdrop-blur-sm">
                                    ...
                                    <div className="space-y-4 mb-10">
                                        <div className="flex items-center justify-between text-[10px] uppercase font-black tracking-widest text-slate-500"><span>Submitted On</span><span className="text-slate-300 italic">{new Date(sub.submittedAt).toLocaleString('en-IN')}</span></div>
                                        <div className="flex items-center justify-between text-[10px] uppercase font-black tracking-widest text-slate-500"><span>Status</span><span className={`px-3 py-1 rounded-full text-[9px] ${sub.status === 'Graded' ? 'bg-emerald-500/20 text-emerald-500 border border-emerald-500/30' : 'bg-brand-primary/20 text-brand-primary border border-brand-primary/30'}`}>{sub.status}</span></div>
                                    </div>
            ...
            <Modal open={!!gradingSubmission} onClose={() => setGradingSubmission(null)} title="Evaluate Assignment">
                <form onSubmit={handleGrade} className="space-y-8">
                    <div className="space-y-3">
                        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 ml-2">Marks Obtained</label>
                        <input type="number" value={gradeForm.score} onChange={(e) => setGradeForm({ ...gradeForm, score: e.target.value })} className="w-full bg-slate-800 border border-slate-700 rounded-md px-6 py-4 text-white focus:outline-none focus:border-brand-primary/50 transition-all font-bold" placeholder="Enter marks..." />
                    </div>
                    <div className="space-y-3">
                        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 ml-2">Teacher Remarks</label>
                        <textarea rows={4} value={gradeForm.feedback} onChange={(e) => setGradeForm({ ...gradeForm, feedback: e.target.value })} className="w-full bg-slate-800 border border-slate-700 rounded-md px-6 py-4 text-white focus:outline-none focus:border-brand-primary/50 transition-all font-medium resize-none" placeholder="Enter feedback..." />
                    </div>
                    <button type="submit" className="w-full py-5 bg-brand-primary hover:bg-teacher-primary rounded-md font-black text-xs uppercase tracking-[0.4em] text-white transition-all shadow-2xl active:scale-95 flex items-center justify-center gap-3"><FileCheck size={18} /> Save Marks & Feedback</button>
                </form>
            </Modal>
        </motion.div>
    );
