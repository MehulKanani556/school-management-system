...
import { fetchMyAttendance, fetchMyLeaves } from '../../redux/slice/staff.slice';
import { 
...
const StaffAttendanceHistory = () => {
    ...
    const { myAttendance, myLeaves, loading } = useSelector((state) => state.staff);
    ...
    const stats = useMemo(() => {
        ...
        const present = myAttendance?.filter(a => a.status === 'Present').length || 0;
        const late = myAttendance?.filter(a => a.status === 'Late').length || 0;
        const absent = myAttendance?.filter(a => a.status === 'Absent').length || 0;
    ...
    const statusConfig = {
        ...
        'Leave': { icon: CalendarIcon, color: 'text-slate-400', bg: 'bg-slate-400/10', border: 'border-slate-400/20', shadow: 'shadow-slate-500/20' }
    };
    ...
    const roleColor = user?.role === 'Accountant' ? 'text-amber-500' : (user?.role === 'Transport_Manager' ? 'text-orange-500' : 'text-teal-500');

    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8 font-outfit">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-10 bg-slate-900/40 p-12 rounded-3xl border border-white/5 shadow-2xl backdrop-blur-3xl ring-1 ring-white/10">
                <div className="space-y-4">
                    <div className="flex items-center gap-4 mb-4">
                        <span className={`w-16 h-[2px] ${roleColor.replace('text', 'bg')} rounded-full`}></span>
                        <span className={`text-[10px] font-black uppercase tracking-[0.5em] ${roleColor}`}>Presence Node</span>
                    </div>
                    <h1 className="text-5xl font-black text-white italic uppercase tracking-tighter leading-none">My Attendance</h1>
                    <div className="flex items-center gap-4 py-2 px-6 bg-white/[0.03] rounded-2xl border border-white/5 w-fit group">
                        <div className={`w-10 h-10 rounded-full ${roleColor.replace('text', 'bg')}/10 flex items-center justify-center ${roleColor}`}>
                            <Activity size={20} className="animate-pulse" />
                        </div>
                        <p className="text-slate-500 font-bold text-sm tracking-wide italic">Personal arrival logs & institutional participation telemetry.</p>
                    </div>
                </div>
            ...
            <div className="bg-slate-900/40 backdrop-blur-3xl border border-white/5 rounded-[3rem] p-12 shadow-2xl ring-1 ring-white/10">
                <div className="flex items-center justify-between mb-16">
                    <div className="flex items-center gap-10">
                        <div className={`w-24 h-24 rounded-[2rem] ${roleColor.replace('text', 'bg')}/10 border border-white/10 flex items-center justify-center ${roleColor} shadow-2xl ring-1 ring-white/5`}>
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.5em]">Temporal Presence Matrix</p>
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center gap-4 bg-slate-950/80 p-4 rounded-3xl border border-white/10 shadow-3xl">
                        <button onClick={() => setCurrentMonth(currentMonth.clone().subtract(1, 'month'))} className="p-5 hover:bg-white/5 rounded-2xl transition-all text-slate-400 hover:text-white group"><ChevronLeft size={28} /></button>
                        <button onClick={() => setCurrentMonth(moment())} className="px-12 py-5 bg-white/5 hover:bg-white/10 rounded-2xl text-[10px] font-black uppercase text-white transition-all tracking-[0.4em] border border-white/5">Today</button>
                        <button onClick={() => setCurrentMonth(currentMonth.clone().add(1, 'month'))} className="p-5 hover:bg-white/5 rounded-2xl transition-all text-slate-400 hover:text-white group"><ChevronRight size={28} /></button>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-7 gap-10">
                    ...
                    {calendarGrid.flat().map((date, i) => {
                        ...
                        let record = myAttendance?.find(a => moment(a.date).isSame(date, 'day'));
                        ...
                        return (
                            <motion.div 
                                ...
                                className={`relative aspect-square rounded-2xl p-6 cursor-pointer transition-all duration-500 group border flex flex-col items-center justify-center overflow-hidden ${isToday ? 'bg-white/5 border-white/20 ring-2 ring-white/10 shadow-2xl' : 'bg-slate-950/40 border-white/5 hover:border-white/10'} ${config ? `${config.bg} ${config.border} ${config.shadow}` : ''}`}
                            >
                                ...
                                <div className="flex flex-col items-center gap-6">
                                    {config ? (
                                        <>
                                            <div className={`w-16 h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center ${config.color} shadow-2xl`}>
                                                <Icon size={24} strokeWidth={2.5} />
                                            </div>
                                            <div className="text-center space-y-1">
                                                <p className={`text-[12px] font-black uppercase tracking-widest ${config.color}`}>{record.status}</p>
