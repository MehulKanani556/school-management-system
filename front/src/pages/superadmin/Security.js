    ...
    if (!security || loading) {
        return (
            <div className="flex flex-col items-center justify-center h-[60vh] opacity-30 grayscale animate-pulse">
                ...
                <h2 className="text-xl font-black uppercase italic tracking-widest text-slate-500">Loading security data...</h2>
            </div>
        );
    }
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-2">
                <div className="flex flex-col text-left">
                    ...
                    <p className="text-[11px] xs:text-sm font-medium text-slate-500 mt-1 tracking-wide flex items-center gap-3">
                        ...
                        Security Monitoring Active. Threat level: <span className={`${threatColor} font-black italic`}>{threatLevel}</span>
                    </p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                {[
                    { label: 'Platform Logs', value: stats.totalLogs, icon: History, color: 'text-indigo-400 bg-indigo-500/10 border-indigo-500/20', note: 'System Logs', tag: 'Audit' },
                    { label: 'Security Alerts', value: stats.criticalAlerts, icon: AlertTriangle, color: 'text-superadmin-primary bg-superadmin-primary/10 border-superadmin-primary/20', note: 'Critical Issues', tag: 'Threats' },
                    { label: 'Failed Access', value: stats.failedAttempts, icon: ShieldOff, color: 'text-orange-500 bg-orange-500/10 border-orange-500/20', note: 'Failed Login Attempts', tag: 'Intrusions' },
                    { label: 'Status', value: '100%', icon: Globe, color: 'text-emerald-500 bg-emerald-500/10 border-emerald-500/20', note: 'All Systems Operational', tag: 'Health' }
                ].map((s, idx) => (
                    <div key={idx} className="bg-slate-900/40 border border-slate-800/60 p-8 rounded-md backdrop-blur-3xl group hover:border-superadmin-primary/30 transition-all duration-300 relative overflow-hidden">
                        <div className="flex items-center justify-between mb-8">
                            ...
                            <span className="text-[9px] font-black uppercase text-slate-700 tracking-widest italic">{s.tag}</span>
                        </div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-slate-900/30 border border-slate-800/60 rounded-md shadow-2xl overflow-hidden backdrop-blur-3xl group min-h-[500px] flex flex-col">
                <div className="px-8 py-6 border-b border-white/5 bg-white/[0.01] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <h2 className="text-lg font-black italic uppercase tracking-tight text-white leading-none font-inter">{activeTab === 'registry' ? 'Recent Activity Log' : 'Detailed System Logs'}</h2>
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest italic mt-2 opacity-60">List of recent administrative actions.</p>
                    </div>
                    {activeTab === 'logs' && logsPagination && (
                        <div className="flex items-center gap-4">
                            ...
                            <span className="text-[10px] font-black text-white uppercase italic tracking-widest">Page {page} / {logsPagination.pages}</span>
                            <button 
                ...
                <div className="flex-1 overflow-x-auto">
                    <table className="w-full text-left">
                        <thead className="bg-white/5">
                            <tr className="border-b border-white/5">
                                ...
                                <th className="px-8 py-5 text-[9px] font-black text-slate-500 uppercase tracking-widest italic whitespace-nowrap">Action</th>
                                <th className="px-8 py-5 text-[9px] font-black text-slate-500 uppercase tracking-widest italic">Module / Category</th>
                                <th className="px-8 py-5 text-[9px] font-black text-slate-500 uppercase tracking-widest text-right italic">Timestamp</th>
                            </tr>
                        </thead>
                                    ...
                                    <td className="px-8 py-5">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className="flex flex-col min-w-0">
                                                ...
                                                <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest italic truncate">{log.userId?.role?.replace('_', ' ') || 'SYSTEM'}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-8 py-5">
                    ...
                    {(displayLogs.length === 0) && (
                        <div className="p-20 text-center flex flex-col items-center justify-center opacity-30 grayscale group hover:grayscale-0 transition-all flex-1">
                             ...
                             <h4 className="text-lg font-black uppercase italic tracking-widest text-slate-500">No Logs Found</h4>
                        </div>
                    )}
                </div>
            </motion.div>
