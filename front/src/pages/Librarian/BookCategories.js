    ...
    return (
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 pb-10 font-outfit">
            <div className="flex justify-between items-end">
                <div>
                    <h1 className="text-3xl font-black text-violet-500 italic uppercase tracking-tighter mb-1 leading-none">Categorical Matrix</h1>
                    ...
                    <div className="bg-neutral-900 border border-slate-800/60 rounded-md p-6 shadow-xl">
                        ...
                        <div className="space-y-2">
                            <button 
                                ...
                                className={`w-full flex items-center justify-between px-4 py-3 rounded-md transition-all group ${selectedCategory === 'all' ? 'bg-violet-600 text-white shadow-lg' : 'text-slate-400 hover:bg-white/5 hover:text-slate-200'}`}
                            >
                            ...
                            {stats.map((stat, i) => (
                                <button 
                                    ...
                                    className={`w-full flex items-center justify-between px-4 py-3 rounded-md transition-all group ${selectedCategory === stat.name ? 'bg-violet-600 text-white shadow-lg' : 'text-slate-400 hover:bg-white/5 hover:text-slate-200'}`}
                                >
                                    <div className="flex items-center gap-3">
                ...
                <div className="md:col-span-3">
                    <div className="bg-neutral-900 border border-slate-800/60 rounded-md shadow-2xl overflow-hidden min-h-[500px]">
                        <div className="px-6 py-5 border-b border-slate-800/60 bg-neutral-950/40 flex items-center justify-between">
                            <h2 className="text-lg font-black italic uppercase tracking-tight text-slate-100 leading-none">Cluster: <span className="text-violet-500">{selectedCategory}</span></h2>
                        </div>
                        
                        <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-4">
                            {filteredBooks.length > 0 ? filteredBooks.map((book, i) => (
                                <div key={i} className="p-4 bg-neutral-950 border border-slate-800/40 rounded-md hover:border-violet-500/40 transition-all flex items-center gap-4 group/item">
                                    <div className="w-10 h-10 rounded-md bg-neutral-900 flex items-center justify-center text-slate-600 group-hover/item:text-violet-500 transition-colors">
                                        <BookOpen size={18} />
                                    </div>
                                    <div className="flex flex-col flex-1">
                                        <span className="text-xs font-bold text-slate-200 uppercase italic tracking-tight mb-1 group-hover/item:text-violet-400 transition-all">{book.title}</span>
