...
const Messages = () => {
    ...
    return (
        ...
        >
            ...
            <div className={`lg:w-96 flex flex-col bg-[#0f0f12] border border-slate-800/60 rounded-md overflow-hidden shadow-2xl ${selectedChat ? 'hidden lg:flex' : 'flex w-full'}`}>
                <div className="p-8 border-b border-slate-800/60 bg-[#0a0a0c]">
                    <h2 className="text-xl font-black text-white italic uppercase tracking-tighter mb-6 font-outfit">Neural Link</h2>
                    <div className="relative group">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-700 group-focus-within:text-luxury-emerald transition-colors" size={16} />
                        <input 
                            placeholder="SCAN CONTACTS..."
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full bg-slate-950/50 border border-slate-800 h-12 pl-12 pr-4 rounded-md text-[10px] font-black uppercase tracking-widest text-white outline-none focus:border-luxury-emerald placeholder:text-slate-800 italic transition-all"
                        />
                    </div>
                </div>

                <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
                    <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest px-4 mb-3 italic">Prioritized Links</p>
                    {conversations.map(conv => {
                        ...
                        return (
                            <button 
                                ...
                                className={`w-full flex items-center gap-4 p-4 rounded-md transition-all border ${isActive ? 'bg-luxury-emerald/10 border-luxury-emerald/30' : 'bg-transparent border-transparent hover:bg-slate-800/30'}`}
                            >
            ...
            {/* Main Chat Interface */}
            <div className={`flex-1 flex flex-col bg-[#0f0f12] border border-slate-800/60 rounded-md overflow-hidden shadow-2xl relative ${!selectedChat ? 'hidden lg:flex' : 'flex'}`}>
                {selectedChat ? (
                    <>
                        <header className="p-8 border-b border-slate-800/60 flex items-center justify-between bg-[#0a0a0c]">
                            <div className="flex items-center gap-5">
                                ...
                                <div className="w-12 h-12 rounded-md bg-slate-800 overflow-hidden border border-luxury-emerald/20 shadow-xl">
                                ...
                                <div>
                                    ...
                                    <span className="flex items-center gap-2 text-[9px] font-black text-luxury-emerald uppercase tracking-widest italic leading-none">
                                        <ShieldCheck size={10} /> Authorized Authority Channel
                                    </span>
                                </div>
                            </div>
                        ...
                        <div ref={scrollRef} className="flex-1 overflow-y-auto p-10 space-y-8 custom-scrollbar bg-slate-950/20">
                                ...
                                return (
                                    ...
                                    >
                                        <div className={`max-w-[70%] space-y-2 ${isMe ? 'items-end' : 'items-start'} flex flex-col`}>
                                            <div className={`p-5 rounded-md text-sm font-medium shadow-xl relative ${isMe ? 'bg-luxury-emerald text-black rounded-tr-none' : 'bg-slate-800 text-slate-200 rounded-tl-none border border-slate-700/50'}`}>
                            ...
                            {activeConversation.messages.length === 0 && (
                                <div className="h-full flex flex-col items-center justify-center opacity-20 gap-6">
                                    ...
                                    <p className="text-[10px] font-black uppercase tracking-[0.4em] italic">No archived transmissions. Initiate uplink.</p>
                                </div>
                            )}
                        </div>

                        <div className="p-8 border-t border-slate-800/60 bg-[#0a0a0c]">
                            <form onSubmit={handleSend} className="flex items-center gap-4 bg-slate-950/50 border border-slate-800 rounded-md p-2 focus-within:border-luxury-emerald transition-all">
                                ...
                                <button 
                                    type="submit"
                                    className="bg-luxury-emerald text-black p-4 rounded-md shadow-lg shadow-luxury-emerald/20 hover:scale-105 active:scale-95 transition-all"
                                >
                                    <Send size={18} />
                                </button>
                            </form>
                    ...
                    <div className="h-full flex flex-col items-center justify-center p-20 text-center space-y-10">
                        ...
                        <div>
                            <h3 className="text-xl font-black text-white uppercase italic tracking-widest leading-none mb-4 font-outfit">Select Link Node</h3>
                            <p className="text-[9px] font-black text-slate-600 uppercase tracking-[.2em] max-w-xs mx-auto leading-relaxed">Establish an encrypted neural bridge with institutional authority nodes.</p>
                        </div>
                    </div>
                )}
            </div>
