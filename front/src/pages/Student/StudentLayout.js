...
const StudentLayout = () => {
  ...
  return (
    <div className="h-screen bg-slate-900 text-slate-100 flex font-inter antialiased overflow-hidden">
      {/* Sidebar - Persistent layout with high-end aesthetic */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-slate-900 border-r border-slate-800/60 flex flex-col transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static lg:h-full`}>
        <div className="p-8 flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-md bg-gradient-to-br from-brand-primary to-brand-secondary flex items-center justify-center font-black text-xl italic shadow-lg">ST</div>
            <span className="text-xl font-black tracking-tight uppercase font-outfit leading-none">Scholar <span className="text-brand-primary">Node</span></span>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
        {/* Header - Stays at top */}
        <header className="h-20 flex-shrink-0 flex items-center justify-between px-8 bg-slate-900/60 backdrop-blur-xl border-b border-slate-800/40 z-10 w-full">
          <div className="flex items-center gap-4 text-slate-500">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest bg-slate-800 px-3 py-1 rounded-md border border-slate-700/50 hidden sm:block leading-none">Scholar Intelligence</span>
            <ChevronRight size={14} className="hidden sm:block" />
            <span className="text-[10px] font-black uppercase tracking-widest text-brand-primary">Student Terminal</span>
          </div>
            ...
            <div className="flex items-center gap-4 relative">
              ...
              >
                <div className="text-right hidden sm:block">
                  ...
                  <p className="text-[9px] font-black text-brand-primary uppercase tracking-[0.2em] mt-1.5 opacity-80 leading-none italic">Institutional Scholar</p>
                </div>
