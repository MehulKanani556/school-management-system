...
const connectDb = async () => {
  try {
    ...
    console.log('✅ MongoDB connected for staff attendance migration');
  } catch (error) {
...
const migrateStaffAttendance = async () => {
  try {
    ...
    for (const school of schools) {
      ...
      // Find staff attendance records without academicYearId for this school
      const recordsToUpdate = await StaffAttendance.find({
        schoolId: school._id,
        academicYearId: { $exists: false }
      });

      if (recordsToUpdate.length === 0) {
        console.log(`   ✅ No staff attendance records to update for this school`);
        continue;
      }

      console.log(`   👥 Found ${recordsToUpdate.length} staff attendance records to update`);

      // Update staff attendance records
      const result = await StaffAttendance.updateMany(
        {
          schoolId: school._id,
          academicYearId: { $exists: false }
        },
      ...
      console.log(`   ✅ Updated ${result.modifiedCount} staff attendance records`);
      totalUpdated += result.modifiedCount;
    }

    console.log(`\n✅ Migration complete!`);
    console.log(`📊 Total staff attendance records updated: ${totalUpdated}\n`);
